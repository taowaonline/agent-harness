...
from ai_harness.evals import _RunnerError, run_eval  # noqa: E402
...
class TomlRunnerEndToEndTests(unittest.TestCase):
    ...
    def test_toml_config_with_runner_executes_and_grades(self) -> None:
        (self.dir / "harness.toml").write_text(textwrap.dedent(f"""
            version = 1
            [project]
            name = "demo"
            language = "python"
        ...
        (self.dir / "ds.jsonl").write_text("\n".join([
            json.dumps({"id": "p1", "input": {"query": "hello"},
...
class MalformedRunnerBlocksTests(unittest.TestCase):
    ...
    def _cfg(self, runner_path: str) -> Config:
        (self.dir / "ds.jsonl").write_text("\n".join([
            json.dumps({"id": "a", "input": {"query": "x"},
                        "expected": {"contains": ["x"]}}),
            json.dumps({"id": "b", "input": {"query": "y"},
                        "expected": {"contains": ["y"]}}),
        ]) + "\n", encoding="utf-8")
        return Config(
            version=1,
    ...
    def test_malformed_json_blocks_stage(self) -> None:
        runner = self._write_runner("bad.py", textwrap.dedent("""
            import sys, json
    ...
    def test_case_id_mismatch_blocks_stage(self) -> None:
        runner = self._write_runner("mismatch.py", textwrap.dedent("""
            import sys, json
            for line in sys.stdin:
                case = json.loads(line)
    ...
    def test_duplicate_case_id_caught_as_mismatch(self) -> None:
        ...
        runner = self._write_runner("dup.py", textwrap.dedent("""
            import sys, json
            for line in sys.stdin:
    ...
    def test_missing_case_id_blocks_stage(self) -> None:
        runner = self._write_runner("noid.py", textwrap.dedent("""
            import sys, json
            for line in sys.stdin:
    ...
    def test_line_count_mismatch_blocks_stage(self) -> None:
        runner = self._write_runner("short.py", textwrap.dedent("""
            import sys, json
